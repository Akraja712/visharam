            ...
            if ($fn->is_item_available($product_id, $product_variant_id)) {
                ...
                if ($stock[0]['stock'] > 0 && $stock[0]['serve_for'] == 'Available') {
                    ...
                    if ($fn->is_item_available_in_user_cart($user_id, $product_variant_id)) {
                        ...
                        $total_quantity = $fn->get_data('cart', "product_id='" . $product_id . "' and user_id='" . $user_id . "' and save_for_later='0'", $columns = ['sum(qty) as total']);
                        if (isset($total_quantity[0]['total']) && !empty($total_quantity[0]['total'])) {
                            ...
                            if (isset($total_allowed_quantity[0]['total_allowed_quantity']) && !empty($total_allowed_quantity[0]['total_allowed_quantity'])) {
                                ...
                                $temp = $fn->get_data('cart', "product_variant_id='" . $product_variant_id . "' and user_id='" . $user_id . "'", $columns = ['qty']);
...
if ((isset($_POST['get_user_cart'])) && ($_POST['get_user_cart'] == 1)) {
    ...
    if ($address_id != "") {
        // print_r("hello");
        $pincodes = $fn->get_data("user_addresses", "id= $address_id AND user_id=$user_id", $column = ['pincode_id', 'pincode']);
        // print_r($pincodes);
        ...
        if ($fn->is_item_available_in_user_cart($user_id)) {
            ...
            if ($shipping_type == 'standard') {
                    ...
                    if (isset($res[$i]['item']) && !empty($res[$i]['item'])) {
                        for ($k = 0; $k < count($res[$i]['item']); $k++) {
                            ...
                            if ($standard_shipping == 1) {
                                $pickup_location = $fn->get_data('pickup_locations', "pickup_location='" . $res[$i]['item'][$k]['pickup_location'] . "'", $column = ['pin_code']);
                                ...
                                if (($pincode_id != 0)) {
                                    ...
                                    if (!empty($user_pincode)) {
                                        ...
                                        if (!empty($pincode_id)) {
                                            $pincodes_data = $fn->get_data("pincodes", "pincode=" . $pincode_id, $column = ['*']);
                ...
                foreach ($res as $row) {
                    ...
                    for ($k = 0; $k < count($res[$i]['item']); $k++) {
                        $res[$i]['item'][$k]['is_item_deliverable'] = false;
                        $pincode_id = $res[$i]['item'][0]['pincodes'];
                        if (!empty($pincode_id)) {
                        ...
                        } else if ($res[$i]['item'][$k]['d_type'] == "all") {
                            $res[$i]['item'][$k]['is_item_deliverable']  = true;
                        } else {
                            $res[$i]['item'][$k]['is_item_deliverable'] = false;
                        }
                        ...
                        $result[$x]['item'][$j]['tax_percentage'] = (empty($result[$x]['item'][$j]['tax_percentage']) or is_null($result[$x]['item'][$j]['tax_percentage'])) ? "0" :  $result[$x]['item'][$j]['tax_percentage'];
            ...
            if ($shipping_type == 'standard') {
                ...
                // $sql_query = "SELECT id FROM user_addresses WHERE user_id ='" . $user_id . "'";
                // $db->sql($sql_query);
                // $res = $db->getResult();

                $check_address = $fn->get_data('user_addresses', "id=$address_id", ['area_id']);
