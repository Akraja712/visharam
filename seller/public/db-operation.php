    ...
    if ($_FILES["upload_file"]["size"] > 0  && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count != 0) {
                ...
                $emapData[23] = trim($db->escapeString($emapData[23])); // weight
                $emapData[24] = trim($db->escapeString($emapData[24])); // height
                $emapData[25] = trim($db->escapeString($emapData[25])); // breadth
                $emapData[26] = trim($db->escapeString($emapData[26])); // length
                $emapData[27] = (!empty($emapData[27]) && $emapData[27] != "") ? trim($db->escapeString($emapData[27])) : ""; // hsn_code
