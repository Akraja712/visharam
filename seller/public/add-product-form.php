        ...
        if ($product_result == 1) {
            if ($_POST['type'] == 'packet') {
                ...
                for ($i = 0; $i < count($_POST['packate_measurement']); $i++) {
                    ...
                    $breadth = $_POST['breadth'][$i];
                    $hsn_code = $_POST['hsn_code'][$i];

                    $sql = "INSERT INTO product_variant (product_id,type,measurement,measurement_unit_id,price,discounted_price,serve_for,stock,stock_unit_id,images,weight,height,length,breadth,hsn_code) VALUES('$product_id','$type','$measurement','$measurement_unit_id','$price','$discounted_price','$serve_for','$stock','$stock_unit_id','$variant_other_images','$weight','$height','$length','$breadth','$hsn_code')";
                    $db->sql($sql);
